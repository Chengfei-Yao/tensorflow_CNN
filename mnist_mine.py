...
def conv2d(x, W):
    """调用tf.nn.conv2d实现步长为1，padding为same的卷积函数"""
    return tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding='SAME')


def max_pool_2x2(x):
    """调用tf.nn.max_pool实现核为2*2,步长为2的最大池化."""
    return tf.nn.max_pool(x, ksize=[1, 2, 2, 1],
                          strides=[1, 2, 2, 1], padding='SAME')


def weight_variable(shape):
    """对给定尺寸的权重进行初始化.tf.truncated_normal为产生正态分布的函数"""
...
def main(_):
    ...
    source_data = np.hstack((train_data, np.transpose([train_labels])))
    next_data = batch_iter(source_data , 50, 20, shuffle=True)
    # 定义占位符
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            batch = next(next_data)
            if i % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={
                    x: batch[:,0:-1], y_: batch[:,-1], keep_prob: 1.0})
                print('step %d, training accuracy %g' % (i, train_accuracy))
            train_step.run(feed_dict={x: batch[:,0:-1], y_: batch[:,-1], keep_prob: 0.5})
